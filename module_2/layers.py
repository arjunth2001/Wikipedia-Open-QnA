...
class BilinearAttention(nn.Module):
    ...
    def forward(self, input, context, context_mask=None, log_probs=True):
        ...
        attn_scores = context.bmm(self.linear(
            input).unsqueeze(dim=2)).squeeze(dim=2)
...
class SequenceAttention(nn.Module):
    ...
    def forward(self, input, context, context_mask=None):
        ...
        if context_mask is not None:
            input_scores.masked_fill_(context_mask.unsqueeze(
                dim=1).expand(input_scores.size()), float('-inf'))
...
class StackedRNN(nn.Module):
    def __init__(
        ...
        self.layers = nn.ModuleList([
            rnn_type(input_size if layer == 0 else (1 + bidirectional)
    ...
    def forward(self, x, mask):
        ...
        # Pack it up
        rnn_input = nn.utils.rnn.pack_padded_sequence(x, lengths.cpu())
        ...
        for rnn in self.layers:
            if self.dropout > 0:
                ...
                rnn_input = nn.utils.rnn.PackedSequence(
                    dropout, rnn_input.batch_sizes)
            rnn_input = rnn(rnn_input)[0]
            rnn_outputs.append(rnn_input)

        # Unpack output
        rnn_outputs = [nn.utils.rnn.pad_packed_sequence(
            o)[0] for o in rnn_outputs]
        output = torch.cat(
            rnn_outputs, dim=2) if self.concat_layers else rnn_outputs[-1]
        ...
        if output.size(1) != mask.size(1):
            padding = output.new(output.size(0), mask.size(
                1) - output.size(1), output.size(2)).zero_()
