...
class ReadingDataset(Dataset):
    def __init__(
        self, args, contexts, examples, dictionary,
        ...
        self.examples = [ex for ex in examples if len(
            ex['answers']['spans']) > 0] if skip_no_answer else examples
        self.feature_dict = self._build_feature_dict(
        ) if feature_dict is None else feature_dict
        ...
        self.answer_texts = {ex['id']: ex['answers']['texts']
                             for ex in self.examples}

        # Calculate sizes for the sampler
        self.question_sizes = np.array(
            [len(ex['question']['tokens']) for ex in self.examples])
        self.context_sizes = np.array(
            [len(self.contexts[ex['context_id']]['tokens']) for ex in self.examples])
    ...
    def _build_feature_dict(self):
        ...
        def maybe_insert(attribute, features):
            if getattr(self.args, attribute, False):
                for feature in features:
                    if feature not in feature_dict:
                        feature_dict[feature] = len(feature_dict)

        maybe_insert('use_in_question', [
                     'in_question_cased', 'in_question_uncased'])
        maybe_insert('use_lemma', ['in_question_lemma'])
        maybe_insert('use_pos', ['pos={}'.format(pos)
                     for ctx in self.contexts for pos in ctx['pos']])
        maybe_insert('use_ner', ['ner={}'.format(ner)
                     for ctx in self.contexts for ner in ctx['ner']])
    ...
    def __getitem__(self, index):
        ...
        question_tokens = torch.LongTensor(
            [self.dictionary.index(w) for w in example['question']['tokens']])
        context_tokens = torch.LongTensor(
            [self.dictionary.index(w) for w in context['tokens']])
        ...
        if len(self.feature_dict) == 0:
            ...
            context_features = torch.zeros(
                self.context_sizes[index], len(feature_dict))
            cased_question = set(example['question']['tokens'])
            uncased_question = set(w.lower()
                                   for w in example['question']['tokens'])
            lemma_question = set(example['question']['lemma'])
            counter = collections.Counter(
                [w.lower() for w in context['tokens']])
            ...
            for i, context_word in enumerate(context['tokens']):
                ...
                if 'pos={}'.format(context['pos'][i]) in feature_dict:
                    context_features[i][feature_dict['pos={}'.format(
                        context['pos'][i])]] = 1.0
                if 'ner={}'.format(context['ner'][i]) in feature_dict:
                    context_features[i][feature_dict['ner={}'.format(
                        context['ner'][i])]] = 1.0
                if 'tf' in feature_dict:
                    context_features[i][feature_dict['tf']] = counter[context_word.lower(
                    )] / self.context_sizes[index]

        if len(example['answers']['spans']) == 0:
            answer_start, answer_end = None, None
        else:
            num_answers = 1 if self.single_answer else len(
                example['answers']['spans'])
            answer_start = [ans[0]
                            for ans in example['answers']['spans'][:num_answers]]
            answer_end = [ans[1]
                          for ans in example['answers']['spans'][:num_answers]]
    ...
    def collater(self, samples):
        ...
        def merge(values):
            ...
            result = values[0].new(
                len(values), max_length, *values[0].size()[1:])
        ...
        def aggregate(list_values):
            ...
            result = list_values[0][0].new(
                len(list_values), seq_length, max_length)
            result = result.fill_(self.dictionary.pad_idx)
        ...
        context_lengths = torch.LongTensor(
            [s['context_tokens'].numel() for s in samples])
...
class BatchSampler(Sampler):
    def __init__(self, dataset, max_tokens=None, batch_size=None, shuffle=True, seed=42):
        ...
        self.batch_size = batch_size if batch_size is not None else float(
            'Inf')
        self.max_tokens = max_tokens if max_tokens is not None else float(
            'Inf')
    ...
    def _batch_generator(self):
        ...
        indices = indices[np.argsort(
            self.dataset.question_sizes[indices], kind='mergesort')]
        indices = indices[np.argsort(
            self.dataset.context_sizes[indices], kind='mergesort')]
